...
#           before do
#             # Create a user record in the database and store it in an instance variable
#             @user = User.create(@doctor_params[:user])
#           end

#           # Use the generated ID directly
#           let(:id) { @user.id }
...
#           # Use the updated user params in the request body
#           let(:user) { { user: { name: 'Updated User' } } }

#           run_test! do |response|
#             expect(response).to have_http_status(:ok)
#             expect(JSON.parse(response.body)).to include(
#               'status' => { 'code' => 200, 'message' => 'User Updated Successfully!', 'data' => an_instance_of(Hash) }
#             )
#           end
#         end
